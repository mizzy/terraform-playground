...
resource "aws_lambda_permission" "invoke_rds_slowquery_to_es_from_slowquery_source_slowquery" {
  ...
  source_arn    = data.aws_cloudwatch_log_group.slowquery_source_slowquery.arn
}
...
resource "aws_cloudwatch_log_subscription_filter" "rds_slowquery_to_es" {
  ...
  log_group_name  = data.aws_cloudwatch_log_group.slowquery_source_slowquery.name
  filter_pattern  = ""
